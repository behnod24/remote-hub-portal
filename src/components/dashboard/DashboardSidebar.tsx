      ...
      <div className="flex h-full min-h-[700px] flex-col justify-between bg-black p-4">
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            >
              <Menu className={`text-white ${isSidebarCollapsed ? 'h-6 w-6' : 'h-6 w-6'}`} />
            </button>
          </div>

          <nav className="flex flex-col gap-2">
            <Link to="/dashboard" 
              className={`flex items-center gap-3 rounded-full bg-[#292929] px-3 py-2 ${
                isSidebarCollapsed ? 'justify-center' : ''
              }`}
            >
              <LayoutDashboard className={`text-white ${isSidebarCollapsed ? 'h-6 w-6' : 'h-6 w-6'}`} />
              {!isSidebarCollapsed && (
